...
def checkinputrange(xname,xval,xmin,xmax):
    if xval>xmax:
        print(f'Option xval with value {xval} is greater than {xmax}, which is not recommended')
    if xval<xmin:
        print(f'Option xval with value {xval} is less than {xmin}, which is not recommended')
...
def varpro2(y,t,phi=[],dphi=[],m=[],n=[],iss=[],ia=[],alpha_init=[],opts=[]):
    ...
    for itern in range(maxiter):
        ...
        _,_,_,work,_= sla.lapack.zgeqp3(djacmat)
        djacout,jpvt,tau,_,_ = sla.lapack.zgeqp3(djacmat,work)
        rjac=np.triu(djacout)
        lwork = sla.lapack.zunmqr('L','C',djacout,tau,res.ravel(order='F')[:,None],-1)[1]
